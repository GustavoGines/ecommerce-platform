...
@if($theme === 'luxury')
    {{-- =========================================================
         ...
         ========================================================= --}}
    <div class="bg-[#030712] min-h-screen text-white dark:text-black pb-24 -mt-16 pt-24" x-data="{ scrolled: false }" @scroll.window="scrolled = (window.pageYOffset > 50)">
        ...
        <div class="sticky top-16 z-40 bg-[#030712]/80 backdrop-blur-xl border-b border-white/5 transition-all duration-300" :class="{'shadow-lg shadow-[var(--color-primary-glow)]': scrolled}">
            <div class="max-w-7xl mx-auto px-6 lg:px-8 py-4 flex items-center justify-between">
                <div class="flex items-center space-x-2 text-xs font-bold uppercase tracking-widest text-gray-500 dark:text-gray-400 dark:text-gray-500">
                    <a href="{{ route('shop') }}" wire:navigate class="hover:text-white dark:text-black transition-colors">Tienda</a>
                    <span class="text-white dark:text-black/20">/</span>
                    @if($product->category)
                        <a href="{{ route('shop', ['categoria' => $product->category->name]) }}" wire:navigate class="hover:text-white dark:text-black transition-colors cursor-pointer">{{ $product->category->name }}</a>
                    @else
                        <span class="text-gray-400 dark:text-gray-500">General</span>
                    @endif
                </div>
        ...
        <div class="max-w-7xl mx-auto px-6 lg:px-8 mt-12">
            <div class="grid grid-cols-1 lg:grid-cols-12 gap-16 relative">
                ...
                <div class="lg:col-span-7">
                    <div class="sticky top-32">
                        ...
                        <div class="relative w-full aspect-[4/3] rounded-3xl bg-[#0a0f1c] border border-white/5 overflow-hidden group flex items-center justify-center p-12 cursor-crosshair"
                            ...
                            @else
                                <svg class="h-32 w-32 text-white dark:text-black/10" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                        ...
                        <div class="mt-16">
                            ...
                            <p class="text-xl text-gray-400 dark:text-gray-500 font-light leading-relaxed">
                                {{ $product->description }}
                            </p>
                        </div>
                    </div>
                ...
                <div class="lg:col-span-5">
                    <div class="sticky top-32 bg-white dark:bg-zinc-900/5 backdrop-blur-2xl border border-white/10 rounded-[2rem] p-10 shadow-[0_0_50px_rgba(0,0,0,0.5)]">
                        ...
                        @elseif($product->stock == 999)
                            <div class="inline-flex items-center gap-2 px-3 py-1 bg-blue-500/20 text-blue-400 border border-blue-500/30 text-[10px] uppercase tracking-widest font-bold rounded-full mb-6" title="Producto a pedido">
                                <span class="w-1.5 h-1.5 rounded-full bg-blue-500 animate-pulse"></span> A Pedido
                            </div>
                        ...
                        <h1 class="text-4xl md:text-5xl font-black text-white dark:text-black tracking-tight leading-none mb-4">{{ $product->name }}</h1>
                        
                        {{-- Price Section --}}
                        <div class="mt-10 mb-10 pb-10 border-b border-white/10">
                            <p class="text-sm font-bold uppercase tracking-wider text-gray-500 dark:text-gray-400 dark:text-gray-500 mb-2">Precio de Lista</p>
                            <div class="flex items-end gap-4">
                                <span class="text-5xl font-black tracking-tighter text-white dark:text-black">${{ number_format($product->retail_price, 2) }}</span>
                                <span class="text-gray-500 dark:text-gray-400 dark:text-gray-500 mb-2 font-medium">Final</span>
                            </div>

                            <div class="mt-6 group cursor-help p-5 rounded-2xl bg-white dark:bg-zinc-900/5 border border-white/5 hover:border-[var(--color-primary)]/50 transition-all duration-300">
                                ...
                                <div class="flex justify-between items-end">
                                    <span class="text-xs text-gray-400 dark:text-gray-500 uppercase tracking-wider">Mínimo {{ \App\Services\PricingService::GLOBAL_WHOLESALE_MIN }} uds. en total</span>
                        ...
                        <div class="border-t border-white/10" x-data="{ active: null }">
                            ...
                            <div class="border-b border-white/10">
                                ...
                                <div x-show="active === 1" x-collapse x-cloak>
                                    <div class="pb-5 text-sm text-gray-400 dark:text-gray-500 font-light leading-relaxed">
            ...
            <div class="mt-32 pt-24 border-t border-white/5">
                <div class="grid grid-cols-1 lg:grid-cols-3 gap-12">
                    <div>
                        <h3 class="text-3xl font-black text-white dark:text-black tracking-tight mb-4">Especificaciones<br><span class="text-gray-600 dark:text-gray-300">Técnicas</span></h3>
                        <p class="text-gray-400 dark:text-gray-500 font-light">Diseñado con precisión milimétrica para ofrecer el máximo rendimiento sin compromisos térmicos.</p>
                    </div>
                    <div class="lg:col-span-2">
                        <div class="divide-y divide-white/5 border-t border-b border-white/5">
                            @foreach($product->technical_specs as $key => $value)
                                <div class="py-6 grid grid-cols-1 md:grid-cols-3 gap-4 group hover:bg-white dark:bg-zinc-900/5 transition-colors px-4 -mx-4 rounded-xl">
                                    <dt class="text-xs font-bold text-gray-500 dark:text-gray-400 dark:text-gray-500 uppercase tracking-widest">{{ $key }}</dt>
                                    <dd class="text-base text-gray-200 font-medium md:col-span-2 group-hover:text-white dark:text-black transition-colors">{{ $value }}</dd>
                                </div>
                            @endforeach
                        </div>
                    </div>
            ...
            <div class="mt-32 pt-24 border-t border-white/5">
                <h3 class="text-2xl font-black text-white dark:text-black tracking-tight mb-10 flex items-center justify-between">
                    Completá tu Setup
                    <a href="{{ route('home') }}#catalog" class="text-[10px] uppercase tracking-widest text-gray-500 dark:text-gray-400 dark:text-gray-500 hover:text-[var(--color-primary)] transition-colors">Ver Todo</a>
                </h3>
                <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-8">
                    @foreach ($relatedProducts as $related)
                        <a href="{{ route('product.detail', $related->slug) }}" wire:navigate class="group relative flex flex-col bg-white dark:bg-zinc-900/5 backdrop-blur-xl rounded-2xl border border-white/5 overflow-hidden transition-all duration-500 hover:border-[var(--color-primary)]/50 hover:shadow-[0_20px_40px_-15px_var(--color-primary-glow)] hover:-translate-y-2">
                            <div class="relative aspect-[4/3] bg-[#0a0f1c] p-6 flex items-center justify-center border-b border-white/5 overflow-hidden">
                                ...
                                @endif
                                <div class="absolute inset-0 bg-black dark:bg-white/60 opacity-0 group-hover:opacity-100 transition-opacity duration-300 flex items-center justify-center">
                                    <span class="text-xs font-bold text-white dark:text-black uppercase tracking-widest bg-white dark:bg-zinc-900/10 backdrop-blur-md px-4 py-2 rounded-full">Ver Detalles</span>
                                </div>
                            </div>
                            <div class="p-6">
                                <h4 class="font-bold text-white dark:text-black truncate transition-colors">{{ $related->name }}</h4>
                                <div class="mt-2 text-gray-400 dark:text-gray-500 font-bold">
            ...
            <div class="mt-24 pt-24 border-t border-white/5">
                <h3 class="text-2xl font-black text-white dark:text-black tracking-tight mb-10 flex items-center justify-between">
                    ...
                    <a href="{{ route('shop') }}" wire:navigate class="text-[10px] uppercase tracking-widest text-gray-500 dark:text-gray-400 dark:text-gray-500 hover:text-[var(--color-primary)] transition-colors">Ir al Catálogo</a>
                </h3>
                <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-8">
                    @foreach ($recentlyViewedProducts as $viewed)
                        <a href="{{ route('product.detail', $viewed->slug) }}" wire:navigate class="group relative flex flex-col bg-white dark:bg-zinc-900/5 backdrop-blur-xl rounded-2xl border border-white/5 overflow-hidden transition-all duration-500 hover:border-[var(--color-primary)]/50 hover:shadow-[0_20px_40px_-15px_var(--color-primary-glow)] hover:-translate-y-2">
                            <div class="relative aspect-[4/3] bg-[#0a0f1c] p-6 flex items-center justify-center border-b border-white/5 overflow-hidden">
                                @if($viewed->image_url)
                            ...
                            <div class="p-6">
                                <h4 class="font-bold text-white dark:text-black truncate transition-colors">{{ $viewed->name }}</h4>
                                <div class="mt-2 text-gray-400 dark:text-gray-500 font-bold">
